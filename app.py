...
def safe_unique(series):
    ...
    for x in series.dropna().unique():
        s=str(x).strip()
        if s and s not in {'N/A','nan','None'}: vals.append(s)
    return sorted(vals)
...
def boiler_pool(df, heating_system):
    ...
    if 'system_type' in pool.columns:
        ...
        hot_mask=pool.system_type.astype(str).str.contains('Hot Water|Hydronic|Water',case=False,na=False)
        if heating_system=='Steam' and steam_mask.any(): return pool[steam_mask]
        if heating_system=='Hot Water' and hot_mask.any(): return pool[hot_mask]
    return pool
...
def equipment_row(comp,m):
    if m.empty:
        return {'Component':comp,'Qty':'','Manufacturer':'','Model #':'No match','Part #':'No match','Pipe Size':'N/A','BTU Range':'No matching range','Description':'Add a matching rule.'}
    r=m.iloc[0]
...
def render_steam(diagram_path):
    if not diagram_path.exists(): st.warning('Steam diagram GIF is missing. Keep steam_heating_system_green_bubbles_return_point_lowered.gif in this folder.'); return
...
for c in ['component','manufacturer','boiler_type','system_type','connection_type','fuel_type','flue_type','tankless_coil','selection_option','pipe_size','draft_hood_style','model_number','part_number','description']:
...
with st.sidebar:
    ...
    mixing_valve_manufacturer=None; mixing_valve_connection_size=None; mixing_valve_connection_type=None
    if heating_system=='Hot Water':
        if fuel=='Oil':
...
if heating_system=='Hot Water':
    ...
    if hi=='Boiler': title,body=boiler_title_body(m,hi,btu,fuel)
        ...
        item=steam_accessories(fuel)[0]; title='STEAM WATER FEEDER'; body=f"{item['Manufacturer']}\\n{item['Model #']}\\nPART # {item['Part #']}"
    else:
        item=steam_accessories(fuel)[1]; title='BACKFLOW PREVENTER'; body=f"{item['Manufacturer']}\\n{item['Model #']}\\nPART # {item['Part #']}"

left,right=st.columns([1.65,1])
with left:
    render_steam(STEAM_DIAGRAM_GIF) if heating_system=='Steam' else render_hot_water(HOT_WATER_DIAGRAM_GIF,hi,title,body)
with right:
...
if heating_system=='Steam' and hi in {'Steam Water Feeder','Backflow Preventer'}: st.dataframe(sel[sel.Component==hi],use_container_width=True,hide_index=True)
