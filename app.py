...
DATABASE_FILE=Path('hydronic_parts_database.csv'); DIAGRAM_GIF=Path('hot_water_hydronic_system_selector_demo.gif')
IMG_W,IMG_H=980,586
...
def select_product(df, component, btu, expansion_system_type=None, pump_connection_type=None, boiler_fuel_type=None, boiler_flue_type=None, boiler_tankless_coil=None, fill_valve_option=None):
    ...
    if component=='Boiler':
        ...
        if boiler_fuel_type=='Oil': f=f[(f.flue_type==boiler_flue_type)&(f.tankless_coil==boiler_tankless_coil)]
    if component==FILL_COMPONENT: f=f[f.selection_option==fill_valve_option]
    return f[(f.min_btu<=btu)&(f.max_btu>=btu)].sort_values(['min_btu','max_btu'],ascending=[False,True])
def make_row(comp,match,expansion_system_type,pump_connection_type,boiler_fuel_type,boiler_flue_type,boiler_tankless_coil,fill_valve_option):
    if match.empty: return {'Component':comp,'Selection Option':fill_valve_option if comp==FILL_COMPONENT else 'N/A','Qty':'','Manufacturer':'','Fuel Type':boiler_fuel_type if comp=='Boiler' else 'N/A','Input MBH':'N/A','Flue Type':boiler_flue_type if comp=='Boiler' and boiler_fuel_type=='Oil' else 'N/A','Tankless Coil':boiler_tankless_coil if comp=='Boiler' and boiler_fuel_type=='Oil' else 'N/A','Connection Type':pump_connection_type if comp=='Pump Isolation Flanges' else 'Any','System Type':expansion_system_type if comp=='Expansion Tank' else 'Any','Model #':'No match','Part #':'No match','Pipe Size':'N/A','BTU Range':'No matching range','Description':'Add a matching rule to the product database.'}
    r=match.iloc[0]
...
if calibration_mode:
    with st.sidebar:
        st.subheader('Highlight Calibration'); pos['cx']=st.slider('Circle center X',0,IMG_W,int(pos['cx'])); pos['cy']=st.slider('Circle center Y',0,IMG_H,int(pos['cy'])); pos['r']=st.slider('Circle radius',10,120,int(pos['r'])); pos['callout_x']=st.slider('Callout X',0,IMG_W,int(pos['callout_x'])); pos['callout_y']=st.slider('Callout Y',0,IMG_H,int(pos['callout_y'])); st.code(f"'{highlighted_component}': {{'cx': {pos['cx']}, 'cy': {pos['cy']}, 'r': {pos['r']}, 'callout_x': {pos['callout_x']}, 'callout_y': {pos['callout_y']}}}")
...
with right:
    ...
    if selected is None: st.warning(f'No match found for {highlighted_component}, {int(btu):,} BTU.')
        ...
        row=selected_equipment[selected_equipment.Component==highlighted_component].iloc[0]; st.success(f"{row['Manufacturer']} {row['Model #']}")
        for key,val in row.items(): st.markdown(f"**{key}:** {val}")
st.subheader('Selected Equipment Breakdown'); st.dataframe(selected_equipment,use_container_width=True,hide_index=True); st.download_button('Download Selected Equipment Breakdown',data=selected_equipment.to_csv(index=False),file_name='selected_equipment_breakdown.csv',mime='text/csv')
st.subheader('Available Ranges for Highlighted Component'); st.dataframe(products[products.component==highlighted_component],use_container_width=True,hide_index=True); st.caption('Demo only. Final component selections should be verified against manufacturer submittals, flow rate, pressure drop, temperature, system pressure, code requirements, and project conditions.')
