...
ORDER = ['Boiler', FILL, 'Air Separator', 'Expansion Tank', 'Oil Filter', 'Mixing Valve', 'Pump Isolation Flanges']
...
POS = {
    ...
    'Oil Filter': {'cx': 260, 'cy': 455, 'r': 28, 'callout_x': 285, 'callout_y': 335},
    'Mixing Valve': {'cx': 230, 'cy': 230, 'r': 30, 'callout_x': 255, 'callout_y': 85},
}
...
div[data-baseweb="select"]>div, div[data-baseweb="input"]>div, .stNumberInput input {
    background:#000 !important; color:#39ff55 !important; border:1px solid #39ff55 !important; border-radius:0 !important;
...
def choices(df, component, column, exclude=('N/A', 'Any')):
    vals = df.loc[df.component == component, column].dropna().astype(str).str.strip().unique().tolist()
    return sorted([v for v in vals if v not in exclude and v != ''])


def filt(df, comp, btu, sys_type, conn, fuel, flue, coil, fillopt,
         boiler_manufacturer=None, air_sep_manufacturer=None,
         mix_mfr=None, mix_size=None, mix_type=None,
         oil_filter_manufacturer=None):
    ...
    if comp == 'Air Separator':
        if air_sep_manufacturer:
            f = f[f.manufacturer == air_sep_manufacturer]

    if comp == 'Oil Filter':
        if oil_filter_manufacturer:
            f = f[f.manufacturer == oil_filter_manufacturer]
...
df = pd.read_csv(DB)
# Normalize text to prevent hidden trailing/leading spaces from breaking exact-match filters.
...
st.caption('THE LOOP IS CLOSED. THE CHOICE IS MADE. SCROLL DOWN FOR EQUIPMENT BREAKDOWN')
...
with st.sidebar:
    ...
    boiler_manufacturer = st.selectbox(
        'Boiler Manufacturer',
        choices(df, 'Boiler', 'manufacturer'),
        key='boiler_manufacturer_selector'
    )

    fuel = st.selectbox('Boiler Fuel Type', ['Natural Gas', 'Oil'], key='boiler_fuel_selector')

    flue = 'N/A'
    coil = 'N/A'
    if fuel == 'Oil':
        flue = st.selectbox('Boiler Flue Type', ['Top Flue', 'Rear Flue'], key='boiler_flue_selector')
        coil = st.selectbox('Tankless Coil', ['Without Tankless Coil', 'With Tankless Coil'], key='tankless_coil_selector')

    oil_filter_manufacturer = None
    if fuel == 'Oil':
        st.subheader('Oil Filter')
        _of = df[df.component == 'Oil Filter'].copy()
        if _of.empty:
            st.warning('No Oil Filter rows found in hydronic_parts_database.csv')
        else:
            oil_filter_manufacturer = st.selectbox(
                'Oil Filter Manufacturer',
                sorted(_of.manufacturer.dropna().unique()),
                key='oil_filter_manufacturer_selector'
            )

    air_sep_manufacturer = st.selectbox(
        'Air Separator Manufacturer',
        choices(df, 'Air Separator', 'manufacturer'),
        key='air_separator_manufacturer_selector'
    )

    fillopt = st.selectbox(
        'Fill Valve / Backflow Preventer',
        choices(df, FILL, 'selection_option', exclude=('N/A',)),
        key='fill_valve_selector'
    )

    sys_type = st.selectbox(
        'Expansion Tank System Type',
        choices(df, 'Expansion Tank', 'system_type', exclude=('Any', 'N/A')),
        key='expansion_tank_system_type_selector'
    )

    conn = st.selectbox(
        'Pump Isolation Flange Connection Type',
        ['Press', 'Sweat', 'Threaded'],
        key='pump_flange_connection_type_selector'
    )
    ...
    if coil == 'With Tankless Coil':
        ...
        if _mv.empty:
            ...
            mixing_valve_manufacturer = st.selectbox(
                ...
                sorted(_mv.manufacturer.dropna().unique()),
                key='mixing_valve_manufacturer_selector'
            )
            _mv = _mv[_mv.manufacturer == mixing_valve_manufacturer]
            mixing_valve_connection_size = st.selectbox(
                'Mixing Valve Connection Size',
                sorted(_mv.pipe_size.dropna().unique()),
                key='mixing_valve_connection_size_selector'
            )
            _mv = _mv[_mv.pipe_size == mixing_valve_connection_size]
            mixing_valve_connection_type = st.selectbox(
                'Mixing Valve Connection Type',
                sorted(_mv.connection_type.dropna().unique()),
                key='mixing_valve_connection_type_selector'
            )

    visible_order = [
        c for c in ORDER
        if not (c == 'Mixing Valve' and coil != 'With Tankless Coil')
        and not (c == 'Oil Filter' and fuel != 'Oil')
    ]
...
for comp in visible_order:
    m = filt(df, comp, int(btu), sys_type, conn, fuel, flue, coil, fillopt,
             boiler_manufacturer, air_sep_manufacturer,
             mixing_valve_manufacturer, mixing_valve_connection_size, mixing_valve_connection_type,
             oil_filter_manufacturer)
    if m.empty:
...
if m.empty:
    ...
    if hi == FILL:
        ...
        body = f'{r.manufacturer}\\n{r.model_number}\\n{r.connection_type} / {r.pipe_size} CONNECTION'
    else:
        title = 'AIR SEPARATOR'
...
with left:
    if not DIAGRAM_GIF.exists():
        ...
        html = f"""
        <style>
        .diagram-wrap{{background:#000;border:1px solid #00cc44;box-shadow:0 0 12px rgba(57,255,85,.35)}}
        ...
        <div class='diagram-wrap'><svg class='diagram-svg' viewBox='0 0 {IMG_W} {IMG_H}'>
        <image href='data:image/gif;base64,{gif}' x='0' y='0' width='{IMG_W}' height='{IMG_H}'/>
        <circle class='pulse' cx='{p['cx']}' cy='{p['cy']}' r='{p['r']}'/>
        <foreignObject x='{p['callout_x']}' y='{p['callout_y']}' width='390' height='230'>
        <div xmlns='http://www.w3.org/1999/xhtml' class='callout'><b>{th}</b><br/>{bh}<br/><span>&gt; SELECTED BY INPUTS</span></div>
