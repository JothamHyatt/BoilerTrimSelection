...
PSHT_SERVICE_MODELS={'PSHT30','PSHT60','PSHT90'}
...
st.markdown('<style>.stApp{background:#000}section[data-testid="stSidebar"]{background:#000!important;border-right:1px solid #39ff55}section[data-testid="stSidebar"] label,.stSelectbox label,.stNumberInput label,.stCheckbox label,h1,h2,h3{font-family:"Courier New",monospace!important;color:#39ff55!important}svg,[data-testid*="icon"],[class*="icon"],[class*="Icon"]{font-family:initial!important}div[data-baseweb="select"]>div,div[data-baseweb="input"]>div,.stNumberInput input{background:#000!important;color:#39ff55!important;border:1px solid #39ff55!important;border-radius:0!important;font-family:"Courier New",monospace!important}button{background:#000!important;color:#39ff55!important;border:1px solid #39ff55!important}</style>', unsafe_allow_html=True)
...
def load_products():
    ...
    for c,d in REQUIRED_COLUMNS.items():
        if c not in df.columns:
            df[c]=d
    for c in ['system_type','connection_type','fuel_type','flue_type','tankless_coil','selection_option','pipe_size','input_mbh']:
        df[c]=df[c].fillna(REQUIRED_COLUMNS.get(c,'N/A'))
...
def select_product(df, component, btu, expansion_system_type=None, pump_connection_type=None, boiler_fuel_type=None, boiler_flue_type=None, boiler_tankless_coil=None, fill_valve_option=None):
    ...
    if component=='Expansion Tank':
        f=f[f.system_type==expansion_system_type]
    if component=='Pump Isolation Flanges':
        f=f[f.connection_type==pump_connection_type]
    if component=='Boiler':
    ...
    if component==FILL_COMPONENT:
        f=f[f.selection_option==fill_valve_option]
...
for comp in COMPONENT_ORDER:
    row=make_row(comp,select_product(products,comp,int(btu),expansion_system_type,pump_connection_type,boiler_fuel_type,boiler_flue_type,boiler_tankless_coil,fill_valve_option),expansion_system_type,pump_connection_type,boiler_fuel_type,boiler_flue_type,boiler_tankless_coil,fill_valve_option)
    selected_rows.append(row)
    if comp=='Expansion Tank':
        expansion_selected_model=row['Model #']
...
if selected is None:
    ...
    svc='\n+ [1] WH41672 SERVICE VALVE' if selected.model_number in PSHT_SERVICE_MODELS else ''
