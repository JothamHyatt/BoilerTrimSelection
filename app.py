...
section[data-testid="stSidebar"] label, section[data-testid="stSidebar"] p, section[data-testid="stSidebar"] h1, section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3, .stSelectbox label, .stNumberInput label, .stCheckbox label, h1, h2, h3, .stCaptionContainer { font-family:"Courier New","DejaVu Sans Mono",monospace !important; color:var(--terminal-green) !important; text-shadow:0 0 7px rgba(57,255,85,.8); }
...
button, .stDownloadButton button { background:#000 !important; color:var(--terminal-green) !important; border:1px solid var(--terminal-green) !important; border-radius:0 !important; font-family:"Courier New","DejaVu Sans Mono",monospace !important; text-shadow:0 0 6px rgba(57,255,85,.75); }
</style>
'''
...
def select_product(df, component, btu, system_type=None, connection_type=None, boiler_fuel_type=None, boiler_flue_type=None, boiler_tankless_coil=None, fill_valve_option=None):
    ...
    if component == 'Boiler':
        if boiler_fuel_type:
            filtered = filtered[filtered['fuel_type'] == boiler_fuel_type]
...
with st.sidebar:
    ...
    boiler_fuel_type=st.selectbox('Boiler Fuel Type', boiler_fuel_types)
    boiler_flue_type='N/A'
    boiler_tankless_coil='N/A'
    if boiler_fuel_type == 'Oil':
...
with left:
    if not DIAGRAM_GIF.exists():
        ...
        .callout-html {{ color:#39ff55; background:rgba(0,0,0,.84); border:1px solid #39ff55; padding:8px 11px; line-height:1.15; font-size:15px; text-shadow:0 0 7px #39ff55; box-shadow:0 0 12px rgba(57,255,85,.55); white-space:pre-line; max-width:320px; }}
